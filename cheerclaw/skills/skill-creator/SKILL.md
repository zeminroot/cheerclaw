...
For `cheerclaw`, custom skills should live under the clawspace `skills/` directory (Path.home() / ".cheerclaw" / "skills") so they can be discovered automatically at runtime (for example, `<clawspace>/skills/my-skill/SKILL.md`).
...
```bash
# Use Path.home() / ".cheerclaw" / "skills" for <skills-directory>
scripts/init_skill.py my-skill --path <skills-directory>
scripts/init_skill.py my-skill --path <skills-directory> --resources scripts,references
scripts/init_skill.py my-skill --path <skills-directory> --resources scripts --examples
```
...
Keep frontmatter minimal. In `cheerclaw`, `metadata` and `always` are also supported when needed, but avoid adding extra fields unless they are actually required.
